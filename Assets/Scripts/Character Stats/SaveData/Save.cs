...
public class Save : Singleton<Save>
{
    public void SaveFun()
    {
        if(File.Exists(Application.dataPath + "/Data" + "/Player Data" + "/Data"))
        {
            PlayerData playerData = new PlayerData();
            BinaryFormatter binaryFormatter = new BinaryFormatter();

            FileStream fs = File.Create(Application.dataPath + "/Data" + "/Player Data" + "/Data");
    ...
    public void LoadFun()
    {
        if (File.Exists(Application.dataPath + "/Data" + "/Player Data" + "/Data"))//这里最好还是判断文件是否存在
        {
            //二进制转换器
